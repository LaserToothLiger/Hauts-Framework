	...
	public static class HautsF_COaNN
	{
		...
		public static bool HautsAddRandomTrait_DoEffectPrefix(CompUseEffect_AddRandomTrait __instance, Pawn user)
		{
			if (user != null && user.story != null)
			{
				...
				for (int i = 0; i < allDefsListForReading.Count; i++)
				{
					...
					if (!blacklistedTraits.Contains(iTrait) && iTrait.GetGenderSpecificCommonality(user.gender) > 0f && !user.story.traits.allTraits.Any((Trait t) => t.def.conflictingTraits.Contains(iTrait)) && !TraitModExtensionUtility.IsExciseTraitExempt(iTrait) && !HautsCOaNNUtility.AddRandomTraitExempt(iTrait))
					{
						list.Add(iTrait);
					}
				}
		...
		public static bool HautsTraitReset_DoEffectPrefix(CompUseEffect_TraitReset __instance, Pawn user)
		{
			if (user != null && user.story != null)
			{
				...
				foreach (Trait t in user.story.traits.allTraits)
				{
					if (!backstoryTraits.Contains(t.def) && t.sourceGene == null && !TraitModExtensionUtility.IsExciseTraitExempt(t.def,false) && !HautsCOaNNUtility.TraitResetExempt(t.def))
					{
						traitsToRemove.Add(t);
						if (ModCompatibilityUtility.COaNN_TraitReset_ShouldDoBonusEffect(t.def))
                        {
				...
				ModCompatibilityUtility.COaNN_TraitReset_BonusEffects(user, bonusEffectPrompts);
			}
			CompUseEffect_AddRandomTrait.RefreshPawnStat(user);
			__instance.parent.SplitOff(1).Destroy(DestroyMode.Vanish);
			return false;
